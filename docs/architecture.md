...
```
Hero -> TermsBanner -> SelectedWork -> About -> Music -> Experience -> Apart -> Skills -> Likes -> Footer (Contact)
```
...
`Likes` is the full-bleed wallpaper-and-stickers desk; see
[`decisions.md`](./decisions.md#d30-things-i-like-a-draggable-sticker-desk-replaces-the-light-band).
Contact is the global `Footer` (D29), so it is not in `home.tsx`.

Numbered sections read 01 to 07 down the page: Selected Work (01, `SectionHeader`
prop), About (02, inline eyebrow in `about.tsx`, not a `SectionHeader`),
Experience (03), Apart (04), Skills (05), Likes (06), Contact (07, shown only on
the homepage by `Footer`). The unnumbered interludes (TermsBanner, Music) sit
between them. **If you reorder sections, renumber**, both the
`SectionHeader index` props, About's hardcoded span, and the footer's index.
